...
/*   Updated: 2024/12/16 20:07:42 by jqueijo-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void aim_ray(t_ray *ray)
{
	...
	{
		ray->stepX = -1;
		ray->rayInterX = (ray->cam.pos.x - ray->gridX) * ray->rayNextX;
	}
	else
	{
		ray->stepX = 1;
		ray->rayInterX = (ray->gridX + 1 - ray->cam.pos.x) * ray->rayNextX;
	}
	printf("ray_id_%d: stepX = %d, rayInterX = %f\n", ray->id, ray->stepX, ray->rayInterX);
	if (ray->rayDirY < 0)
	{
		ray->stepY = -1;
		ray->rayInterY = (ray->cam.pos.y - ray->gridY) * ray->rayNextY;
	}
	else
	{
		ray->stepY = 1;
		ray->rayInterY = (ray->gridY + 1 - ray->cam.pos.y) * ray->rayNextY;
	}
...
static void cast_ray(t_game*game, t_ray *ray)
{
	...
	while (ray->hit == 0)
	{
		if (ray->rayInterX < ray->rayInterY)
		{
			...
			ray->gridX += ray->stepX;
			ray->side = 0;
			printf("ray_id_%d: Updated rayInterX = %f, gridX = %d\n", ray->id, ray->rayInterX, ray->gridX);
		}
		else
		{
			printf("ray_id_%d: rayInterX >= rayInterY, rayInterX = %f, rayInterY = %f\n", ray->id, ray->rayInterX, ray->rayInterY);
			...
			ray->gridY += ray->stepY;
			ray->side = 1;
			printf("ray_id_%d: Updated rayInterY = %f, gridY = %d\n", ray->id, ray->rayInterY, ray->gridY);
		}
		if (!game->map[ray->gridY] || !game->map[ray->gridY][ray->gridX]
			|| game->map[ray->gridY][ray->gridX] == '1'
			|| game->map[ray->gridY][ray->gridX] == ' ')
		{
			printf("ray_id_%d: Hit wall or out of bounds at gridX = %d, gridY = %d\n", ray->id, ray->gridX, ray->gridY);
			ray->hit = 1;
		}
	}
	ray->ray_dist[ray->id] = get_wall_distance(ray);
	ray->wall_height[ray->id] = (int)(ray->cam.height / ray->ray_dist[ray->id]);
